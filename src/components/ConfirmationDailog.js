...
export default ConfirmationDailog = React.forwardRef((props, ref) => {
  ...
  const {header, NegText, NegPress, PosText, NegColor, PosPress, data } = propsData
  return (
    <Modal
    ...
    >
      <View style={styles.modalView}>
       ...
       {NegText && <TouchableOpacity style={[styles.negBtn,{ marginRight:WP(3), backgroundColor: Colors.boldBorderColor, borderWidth:  1 , borderColor: Colors.gray }]}
          onPress={()=>{
            ModalVisibility(false)
            NegPress && NegPress(data)
          }}
          >
          <Text style={[styles.okayText, {color:  Colors.gray }]}>{NegText}</Text>
        </TouchableOpacity>}
